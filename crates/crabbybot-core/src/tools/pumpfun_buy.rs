...
impl PumpFunBuyTool {
    ...
    fn get_signer_key(&self) -> Result<ed25519_dalek::SigningKey, String> {
        let key_str = self.private_key.as_ref().ok_or(
            "❌ No Solana private key configured. Use the `onboard` command or edit `config.json`.",
        )?;
...
impl Tool for PumpFunBuyTool {
    ...
    async fn execute(&self, args: HashMap<String, Value>) -> String {
        ...
        let rug_report = match self.rugcheck.fetch_report(mint).await {
            ...
            Err(e) => {
                return format!(
                    "⚠️ Could not perform safety check: {}\nAborting for safety.",
                    e
                )
            }
        };
        ...
        if !confirm {
            let safety_icon = if rug_report.score < 2000 {
                "🟢"
            } else if rug_report.score < 5000 {
                "🟡"
            } else {
                "🔴"
            };
            ...
            let mut summary = format!(
                ...
                rug_report.file_meta.name,
                rug_report.file_meta.symbol,
                amount_sol,
                safety_icon,
                rug_report.score,
                if rug_report.score >= 5000 {
                    "🛑 **HIGH RISK DETECTED**"
                } else {
                    "✅ Ready for execution"
                }
        ...
        debug!(
            mint,
            amount_sol, "Requesting swap transaction from SolanaTracker"
        );
