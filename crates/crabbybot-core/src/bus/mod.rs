...
use events::{InboundMessage, InternalMessage, OutboundMessage};
...
mod tests {
    ...
    async fn test_subscribe_after_creation() {
        ...
        let dispatch_handle =
            tokio::spawn(dispatch_outbound(Arc::clone(&subs), receivers.outbound_rx));
        ...
        bus.publish_outbound(OutboundMessage::reply("late_channel", "c1", "late message"))
            .await;
